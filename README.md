...
Leaflet + OpenStreetMap で経路の形状を地図上に描画する。一覧の行にホバーすると対応する経路がハイライトされる。クリックでハイライトを固定でき、一覧表と地図が連動する。

### 経路登録

バス停名のインクリメンタルサーチで乗車・降車バス停を選択する。検索結果には事業者カラーバッジが表示され、複数事業者が乗り入れるバス停では全事業者のバッジが並ぶ。徒歩時間も設定でき、出発目安の算出に使われる。
...
画面下部の「経路登録」セクションで乗車バス停と降車バス停を検索・選択する。徒歩時間（分）を入力して「登録」を押す。
...
一覧の下に経路マップが表示される。行の上にカーソルを置くと、対応する経路がハイライトされる。行をクリックするとハイライトが固定され、再クリックで解除される。
...
データソースは [HODA（北海道オープンデータポータル）](https://ckan.hoda.jp)の GTFS フィードを使用している。
