...
バス停名のインクリメンタルサーチで乗車・降車バス停を選択する．検索結果には事業者カラーバッジが表示され，複数事業者が乗り入れるバス停では全事業者のバッジが並ぶ．名前に包含関係があり 200m 以内のバス停は自動統合される（例: 旭川駅と旭川駅前）．徒歩時間はデフォルト 10 分で，出発目安の算出に使われる．経路ごとに通知のオン/オフを設定できる．
...
- Node.js 22.22.2 以上．下限は `package.json` の `engines` が持つ
...
HODA はダイヤ改正時に次期データで上書きする．このとき現行ダイヤのデータは失われることがある．これを防ぐため，最新の GTFS データに加えて一期間前のデータをキャッシュで保持する．カレンダーの `start_date` が変化した場合のみデータを更新する．一期間前のデータは，新しいダイヤの開始日を迎えるまでの表示に使う．開始日を過ぎたら現行ダイヤだけを表示する．
