...
一方のバス停を選択すると、もう一方の候補は「乗り換えなしで到達できるバス停」に自動で絞り込まれる。入力した名称が候補に出ない場合は「直通便で到達できない組み合わせ」の可能性を入力欄付近で告知する。登録ボタン押下時のバリデーションは、未選択・存在しない名称・候補から未選択・直通便で到達不可能をそれぞれ区別してエラーメッセージで示す。登録・更新が完了すると成功トーストで通知する。
