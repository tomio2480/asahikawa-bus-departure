...
### 21. 文書の主張は，成り立つ条件まで書く

- **指摘例**（PR #179 Codex）：`.github/dependabot.yml` の挙動を「宣言が既に許す範囲へ収まる更新は提案されない」と書いていた．反例が同じリポジトリにあった．PR #145 は `react` を `^19.2.4` から `^19.2.8` へ上げている．実際に届かないのは「宣言範囲がメジャー 1 つぶん以上遅れている依存の，旧メジャー内の更新」だけである．条件を落とすと，監視の範囲を誤って伝える．
- **根本原因**：1 つの事例（`vitest`）から挙動を一般化した．反例を探さないまま書いた．しかも同じ誤りが 4 か所へ写り，訂正のたびに広がった．
- **対処パターン**：観測 1 件から挙動を書くときは，**反例を先に探す**．探す先は同じリポジトリの過去 PR である．反例が見つからなくても，成り立つ条件を明示して書く．「〜は起きない」より「〜のとき〜は起きない」を選ぶ．
- **補足**：もう 1 つの例は PR #176 Codex の指摘である．`workflow_dispatch` を 60 日無操作停止への備えとして書いたが，無効化は workflow 単位で起こるため備えにならない．**2 つの事実を並べると，因果があるように読める**．備えと目的は分けて書く．
- **チェック観点**：
  - 「〜しない」「〜は届かない」と書いた箇所に，条件が付いているか．
  - その主張の反例を，リポジトリの過去 PR から 1 度は探したか．
  - 同じ主張を 2 か所以上へ書いていないか．書いたなら，訂正時に全箇所を追えるか．
  - 挙動を確かめる先が，結果の有無ではなく実行の記録になっているか．Dependabot なら起票の有無ではなく「Dependabot Updates」workflow の run を見る．
...
- [ ] 「〜しない」と断定した記述に条件を付けたか．反例を過去 PR から探したか．
- [ ] Bot へレビューを頼んだら，`comments` だけでなく `reviews` も引いたか．Codex の返答は review として届く．
...
| #176 | 脆弱性の勧告が届く経路を用意する | `workflow_dispatch` を無操作停止の備えと誤記，`npm audit` が補う範囲の過大表示 |
| #179 | workflow 一覧の欠落を埋め，脆弱性の経路を設計へ残す | 観測 1 件からの一般化．反例が同じリポジトリの PR #145 にあった |
