...
[Issue #133][i133] で npm を Dependabot の追跡へ載せた直後，PR が 5 本届いた．内訳は minor/patch の束が 1 本と major が 4 本である．本書に残すのは，受領の順序をどう組んだか，major の可否を何で判断したか，CI の通過表示だけを根拠にしなかった理由である．
...
ここで 1 本ずつリベースを頼むと，待ち時間が直列に積み上がる．4 本へ一括でリベースを依頼し，`CLEAN` になった順にマージした．
...
判断の要は「破壊的変更の一覧」ではなく「その変更が自分のコードのどこへ当たるか」である．jsdom の CSSOM 刷新は大きな変更だが，スタイルを検査していなければ当たらない．一覧を読むだけでは可否は決まらない．
...
2 つ目は [Issue #132][i132] の回帰確認である．vite が 2 世代上がったため，`public/` の走査時期が変わっていないかを実物で確かめた．CHANGELOG に該当の記載は無かったが，記載が無いことは挙動が同じであることの証明にならない．
