...
| `ACCESSIBILITY.md` | アクセシビリティの準拠目標と見直しの周期 | 年 1 回の見直し時・準拠水準を変える時 |

アクセシビリティは方針と実装で置き場所を分ける．目指す水準そのものは `ACCESSIBILITY.md` が持つ．本書が扱うのは，個々の実装がその形を採る理由に限る．
...
どちらの経路も拾わない穴が 1 つ残る．宣言範囲がメジャー 1 つぶん以上遅れている依存の，旧メジャー内の更新である．Dependabot は宣言範囲の内側で足りる更新も提案し，そのとき宣言の下限を上げる．ただし更新先は 1 つに絞られるため，より新しいメジャーが出ている依存ではそちらが選ばれ，旧メジャー内の patch は出てこない．`vitest` の `^3.0.0` に対する 3.2.7 がこれにあたる．勧告があれば `npm audit` が拾うが，勧告が無ければどちらの経路にも載らない．

判断の経緯と残る穴は [調査記録](notes/2026-09-04-vulnerability-alert-path.md) に残した．
